...
export interface PlannerApi {
  ...
  blockOrder(id: string, reason: string): void;
  unblockOrder(id: string): void;
  confirmDespacho(id: string): void;
...
const STORAGE_KEY = "po-planner-v1";
  ...
  const api = useMemo<PlannerApi>(() => {
    ...
    return {
      updateOrder(id, patch, logText) {
        setState((s) =>
          patchOrder(s, id, (o) => withLog({ ...o, ...patch }, logText ?? "Información del pedido actualizada."))
        );
      },
      ...
      setStatus(id, status) {
        setState((s) =>
          patchOrder(s, id, (o) => {
            ...
            const cleaned =
              o.status === "bloqueado"
                ? { blockReason: undefined, blockedAt: undefined, prevStatus: undefined }
                : {};
            return withLog(
              { ...o, ...cleaned, status },
              `Estado actualizado → ${STATUS_META[status].label}.`
            );
          })
        );
      },

      blockOrder(id, reason) {
        setState((s) =>
          patchOrder(s, id, (o) =>
            withLog(
              {
              ...
              `Pedido bloqueado — motivo: ${reason}.`
            )
          )
        );
      },
        ...
        setState((s) =>
          patchOrder(s, id, (o) => {
            ...
            return withLog(
              ...
              `Bloqueo liberado → ${STATUS_META[back].label}.`
            );
          })
        );
      },
      ...
      confirmDespacho(id) {
        setState((s) =>
          patchOrder(s, id, (o) =>
            withLog(
              { ...o, status: "despacho", progress: 100, archived: true },
              "Pedido finalizado y despachado — sale del backlog, permanece en calendario."
            )
          )
        );
      },
        ...
        setState((s) => {
          ...
          return {
            ...patchOrder(s, orderId, (o) =>
              withLog(o, `${units} uds asignadas al ${fmtMedium(date)}.`)
            ),
        ...
        setState((s) => {
          ...
          const next = {
            ...
            chunks: s.chunks.map((x) =>
              x.id === chunkId ? { ...x, units: clamp(units, 1, 99999) } : x
            ),
          };
          return patchOrder(next, c.orderId, (o) =>
            withLog(o, `Unidades del ${fmtMedium(c.date)} ajustadas a ${units}.`)
          );
        });
      },
      ...
      moveChunk(chunkId, date) {
        setState((s) => {
          ...
          const next = {
            ...s,
            chunks: s.chunks.map((x) => (x.id === chunkId ? { ...x, date } : x)),
          };
          return patchOrder(next, c.orderId, (o) =>
            withLog(o, `Tarjeta de ${c.units} uds movida al ${fmtMedium(date)}.`)
          );
        });
      },
